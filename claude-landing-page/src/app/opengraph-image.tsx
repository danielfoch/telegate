...
async function loadNunito(): Promise<ArrayBuffer | null> {
  try {
    ...
    const url = css.match(/src: url\(([^)]+)\) format\('(?:truetype|opentype|woff)'\)/)?.[1] ?? css.match(/url\(([^)]+\.(?:ttf|woff))\)/)?.[1];
    if (!url) return null;
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", width: "100%" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 26, color: "#9fb0b8" }}>
            <div>Talk it through. Send it to the agents on your computer at home.</div>
