...
run_stack() {
  ...
  rm -f "$URL_FILE"
  trap 'stop_children; exit 0' EXIT INT TERM
  ...
  for _ in {1..45}; do
    if curl -s -m 5 "$url/health" 2>/dev/null | grep -q '"ok"'; then break; fi
    sleep 2
  done
  if curl -s -m 5 "$url/health" 2>/dev/null | grep -q '"ok"'; then
    ...
    log "warning: $url did not answer /health within 90s; publishing it anyway"
    print -r -- "$url" > "$URL_FILE"
  fi
