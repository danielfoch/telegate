...
try {
  ...
  await writeFile('.env',`TELEGATE_DOMAIN=${domain}\nTELEGATE_SIGNUP_CODE=${invite}\nCALLBACK_SIGNING_KEY=${randomBytes(32).toString('base64url')}\nPUSH_ENCRYPTION_KEY=${randomBytes(32).toString('base64')}\n`,{mode:0o600,flag:'wx'});
  console.log('Saved .env with a new pilot invitation code. Point this hostname to your server, then run docker compose up -d --build. Keep .env private.');
}finally{rl.close();}
